...
func (r *SQLiteRepository) GetMessageByID(id string) (*domainChatStorage.Message, error) {
	query := `
		SELECT id, chat_jid, device_id, sender, content, timestamp, is_from_me,
			media_type, call_metadata, filename, url, direct_path, local_media_path, reply_to_message_id, quoted_text, quoted_sender, media_key, file_sha256,
			file_enc_sha256, file_length, referral_metadata, deleted_at, created_at, updated_at
		FROM messages
		WHERE id = ?
		LIMIT 1
	`
...
// GetMessageByIDAndDevice is the upstream device-scoped lookup name.
func (r *SQLiteRepository) GetMessageByIDAndDevice(deviceID, id string) (*domainChatStorage.Message, error) {
	return r.GetMessageByIDByDevice(deviceID, id)
}
...
func (r *SQLiteRepository) StoreMessage(message *domainChatStorage.Message) error {
	...
	result, err := r.db.Exec(`
		UPDATE messages SET sender = ?, content = ?, timestamp = ?, is_from_me = ?,
			media_type = ?, call_metadata = ?, filename = ?, url = ?, direct_path = COALESCE(NULLIF(?, ''), direct_path),
...
func (r *SQLiteRepository) loadMessageReactions(deviceID, chatJID string, messages []*domainChatStorage.Message) error {
	...
	for start := 0; start < len(messageIDs); start += 500 {
		...
		for _, messageID := range messageIDs[start:end] {
...
func (r *SQLiteRepository) scanMessage(scanner interface{ Scan(...any) error }) (*domainChatStorage.Message, error) {
	...
	var callMetadata sql.NullString
	...
	var referralMetadata sql.NullString
	var deletedAt sql.NullTime

	err := scanner.Scan(
		&message.ID, &message.ChatJID, &message.DeviceID, &message.Sender, &content,
		&message.Timestamp, &message.IsFromMe, &mediaType, &callMetadata, &filename,
		&url, &directPath, &localMediaPath, &replyToMessageID, &quotedText, &quotedSender,
		&message.MediaKey, &message.FileSHA256, &message.FileEncSHA256,
		&message.FileLength, &referralMetadata, &deletedAt, &message.CreatedAt, &message.UpdatedAt,
	)
	if err != nil {
		return message, err
	}
	...
	message.MediaType = nullableString(mediaType)
	message.CallMetadata = nullableString(callMetadata)
	...
	message.ReferralMetadata = nullableString(referralMetadata)
	if deletedAt.Valid {
...
func (r *SQLiteRepository) getMigrations() []string {
	return []string{
		...
		// Migration 23: JSON metadata for synthetic call rows (media_type = call)
		`ALTER TABLE messages ADD COLUMN call_metadata TEXT DEFAULT ''`,

		// Migration 24: JSON metadata for Meta Ads referral/attribution (CTWA)
		`ALTER TABLE messages ADD COLUMN referral_metadata TEXT DEFAULT ''`,

		// Migration 25: Store emoji reactions per message
		...
		// Migration 26: Index reactions by message and chat for history hydration
		`CREATE INDEX IF NOT EXISTS idx_message_reactions_lookup ON message_reactions(device_id, chat_jid, message_id)`,

		// Migration 27: Append-only message edit history
		...
		// Migration 36: Persist live Chatwoot forward retries across restarts
		...
		// Migration 47: Scope a message link to the config that produced it
		`ALTER TABLE chatwoot_message_links ADD COLUMN chatwoot_config_id INTEGER NOT NULL DEFAULT 0`,

		// Migration 48: Denormalize Chatwoot account id for scoped reverse lookups
		`ALTER TABLE chatwoot_message_links ADD COLUMN chatwoot_account_id INTEGER NOT NULL DEFAULT 0`,

		// Migration 49: Resolve Chatwoot replies by conversation and account
		...
		// Migration 50: Count and delete message links by owning config
